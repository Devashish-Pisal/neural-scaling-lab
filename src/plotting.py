...
'''
def plot_exponent_generalization_gap(
        all_results,
        csv_config,
        ds_name):
    fig, ax = plt.subplots(
    ...
    for exponent, result in all_results.items():
        if ds_name == "ImageNet":
            ...
            data = result["fornet"]
        gap = (
            data[csv_config.min_train_loss] -
            data[csv_config.min_val_loss]
        )
        all_gap_values.extend(
            gap.values
        )
    show_exact_values(
        ax,
    ...
    ax.set_title(
        f'{ds_name}: under/overtraining diagnostic'
    )
    ax.legend()
    ax.grid(
        True,
    ...
    fig.savefig(
        PDF_OUTPUTS_DIR /
        f'{ds_name.lower()}_generalization_gap.pdf'
    )
    fig.savefig(
        IMG_OUTPUT_DIR /
        f'{ds_name.lower()}_generalization_gap.png'
    )
    plt.show()
'''
